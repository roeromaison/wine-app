...
```
                                    ┌→ Render Static Site … 画面（常に一瞬で表示）
あなたのPC →（アップロード）→ GitHub ┤
                                    └→ Render Web Service … 分析API（初回だけ起動待ち）
```

- **GitHub** … コードの置き場所。無料のWebサービス
- **Render Static Site** … 画面を配信する。無料。**眠らないので待ち時間ゼロ**
- **Render Web Service** … 分析の計算をするサーバー。無料枠あり

同じ Render の中に、種類の違うサービスを2つ作ります。名前が似ていますが、
**画面は「静的サイト」、APIは「Webサービス」**と別物です。

なぜ GitHub が要るのか、と思われるかもしれません。Render は
...
画面を Static Site に分けると、画面は常に一瞬で出ます。静的サイトは眠らないためです。
訪問者はすぐ触れて記録も入力できます。待ちが発生するのは「分析」タブを開いた
初回だけで、そのときはアプリが動いているので **「計算中…」** と表示できます。

実測で画面0.6秒／API復帰31.5秒でした。
...
## 4-B. 画面を Render Static Site に置く

ここからが「画面とAPIを分ける」作業です。4 での公開が終わっている前提です。

画面の設定は `render.yaml`（Blueprint という仕組み）に書いてあるので、
**入力する項目はひとつもありません。** ファイルの通りに作られます。
...
1. Render のダッシュボードで **Add new** → **Blueprint**
2. `wine-app` を選ぶ
3. `render.yaml` の内容が読み込まれ、作られるもの（`wine-app-web` という
   Static Site）が一覧で表示される
4. **Apply** を押す

1〜2分で完了し、`https://wine-app-web.onrender.com` が発行されます。
**これが読者に案内する公開URLです。**

> `render.yaml` に書いてあるのは**画面だけ**です。分析APIは 4 で手作業で
> 作成済みなので、ここに書くと同じものが2つできてしまいます。

### なぜ Vercel ではないのか

以前は画面を Vercel に置く手順にしていました。実際にやってみたところ、
**Root Directory の設定と `vercel.json` の設定が二重に効いてビルドが失敗し**、
やり直すうちに同名のプロジェクトが2つできる事故も起きました。

Render の Static Site でまったく同じこと（眠らない静的ホスティング）ができ、
しかも `render.yaml` に書いておけば手入力がゼロになります。
**サービスをRenderに一本化した方が、覚えることも減ります。**
...
**画面のURL**（`https://wine-app-web.onrender.com`）を開いて、次を確認してください。
...
> 読者に案内するのは **画面のURL**（`wine-app-web`）です。APIのURL
> （`https://wine-app-hp67.onrender.com`）は裏方なので、記事には載せません。
> 名前が似ているので、貼るときに取り違えないよう注意してください。

---

## 6. 無料プランの注意

Static Site（画面）は眠りません。**画面はいつでも一瞬で出ます。**

Web Service（分析API）は15分アクセスがないと眠ります。眠っているときに
...
画面とAPIの**両方が自動で気づいて**作り直します。数分待てば反映されます。
...
**画面のビルドが失敗する**
→ `render.yaml` の `rootDir: frontend` が消えていないか確認してください。
　 画面のソースは `frontend` フォルダの中にあるので、この指定が要ります。
　 直したら push すれば作り直されます。
